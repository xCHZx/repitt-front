...
import { getStampCardByIdAsCurrentCompany, updateStampCardAsCompany, uploadStampCardIcon } from '@/services/company/stampCards'
...
const existingIconPath = ref<string | null>(null)
...
const getData = async () => {
  try {
    const data = await getStampCardByIdAsCurrentCompany(companyStore.selectedCompany.id, route.params.id)
    name.value = data.name ?? ''
    description.value = data.description ?? ''
    requiredStamps.value = data.requiredStamps ?? 5
    requiredHours.value = data.requiredHours ?? 1
    startDate.value = data.startDate ?? ''
    endDate.value = data.endDate ?? ''
    reward.value = data.reward ?? ''
    allowedRepeats.value = data.allowedRepeats ?? 1
    primaryColor.value = data.primaryColor ?? '#493599'
    existingIconPath.value = data.stampIconPath ?? null
  }
  catch {
    // silently ignore
  }
...
const submit = async () => {
  ...
  const stampCardId = route.params.id

  submitError.value = null
  isSubmitting.value = true
  try {
    await updateStampCardAsCompany(businessId, stampCardId, {
      name: name.value,
      description: description.value,
    })

    if (stampIcon.value?.length)
      await uploadStampCardIcon(businessId, stampCardId, stampIcon.value[0])

    router.push(`/empresa/tarjetas/${stampCardId}`)
  }
  catch (error: any) {
    submitError.value = Array.isArray(error) ? error.join('\n') : String(error)
  }
  finally {
...
<template>
  ...
  >
    ...
    <VAlert
      ...
      class="mb-5"
    >
      Solo puedes editar el nombre, descripción, ícono y color. Para cambios adicionales contacta a <strong>soporte@repitt.com</strong>
    </VAlert>
          ...
          <div>
            <div class="text-caption text-medium-emphasis mb-1">
              {{ selectedIconName
                ...
                : customFilePreview ? 'Archivo personalizado'
                : existingIconPath ? 'Ícono actual'
                : 'Sin ícono seleccionado' }}
            </div>
            <VBtn
              variant="tonal"
              size="small"
